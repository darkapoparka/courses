...
## Expanded source families — 2026-09-09

The expanded course UI retains the existing photo-led Home and calibrated learner shell. New surfaces use the same semantic tokens and single icon family: square course identity beside compact actions, readable ordered curriculum rows, image-led creator pages, category/search galleries, grid/list Library states, collection mosaics, concise native forms and Radix dialog/menu behavior. Only implemented destinations are active; selected navigation follows the pathname and Saved query state.

`web/src/app/flows.css` owns these additional course surfaces; `patterns.css` styles isolated reference studies. The lesson workspace is a focused teal media/study composition: native local-video controls, readable transcript, curriculum and secondary study tabs. It has no persistent empty music player, karaoke blur, shuffled curriculum, fake access or autoplaying sound. Original sample captions and worksheet are public and clearly labeled.

Mobile reflows the same information rather than shrinking a desktop screenshot. Thumbnail dimensions are reserved, source portraits cannot stretch table rows, dialogs fit the viewport, and native bottom navigation is hidden in the focused lesson workspace. Desktop/mobile screenshots and 320/768/1920px overflow checks were performed; this does not certify real-device playback or every zoom/screen-reader configuration.

The `/preview/` event/channel/recap/code-entry/language families are requested visual studies, not adoption of those source business features. Their branding, illustrative data, source-art replacement and explicit no-provider boundaries remain visible. `reference-review/flow-coverage.md` and the current comparison gallery distinguish source observations, adaptations and unavailable behavior. No whole-image pixel identity or owner approval is asserted.
