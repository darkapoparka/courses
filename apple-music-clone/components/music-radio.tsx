...
export function RadioView() {
  const m = useMusic();
  const [hoveredStation, setHoveredStation] = useState<string | undefined>();
  const source = m.scene.source?.slice(0, 8);
  const captured = m.scene.radioEdition === "hits" || m.scene.selectedTrack === "station-1" || m.activeId === "station-1" || hoveredStation === "station-1";
  ...
  const selectStation = (stationId: string) => {
    ...
    if (stationId === "station-1" && m.scene.radioEdition !== "hits") m.patch({ radioEdition: "hits" });
  };
  const clearStation = (stationId: string) => {
    setHoveredStation(undefined);
    if (stationId === "station-1" && m.activeId !== stationId && m.scene.radioEdition !== "launch") m.patch({ radioEdition: "launch" });
  };
  return <div className={`page-content radio-page ${styles.page}`} data-edition={captured ? "hits" : "launch"} data-station-menu={m.scene.menu === "station" || undefined}><h1>Radio</h1>
    <Section title="Live Radio"><p className="section-description">Tap a station to hear non-stop music and more.</p><Rail label="Live radio" className="station-rail">{radioStations.slice(0,6).map(station => { const selected = m.scene.selectedTrack === station.id || hoveredStation === station.id; return <article className="station-card" data-selected={selected || undefined} key={station.id}><button type="button" className="card-art-button" aria-label={`Listen to ${station.title}`} onMouseEnter={() => selectStation(station.id)} onMouseLeave={() => clearStation(station.id)} onFocus={() => selectStation(station.id)} onBlur={() => clearStation(station.id)} onClick={() => m.go(station.destination)}><Art art={station.art} label={station.title} /><span className="card-play"><Glyph name="play" /></span></button><IconButton icon="more" label={`${station.title} options`} onClick={event => m.openMenu("station", event, station.id)} /></article>; })}</Rail></Section>
