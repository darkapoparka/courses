...
export function RadioView() {
  const m = useMusic();
  return <div className={`page-content radio-page ${styles.page}`}><h1>Radio</h1>
    <Section title="Live Radio"><p className="section-description">Tap a station to hear non-stop music and more.</p><Rail label="Live radio" className="station-rail">{radioStations.slice(0,6).map(station => <article className="station-card" key={station.id}><button type="button" className="card-art-button" aria-label={`Listen to ${station.title}`} onClick={() => m.go(station.destination)}><Art art={station.art} label={station.title} /><span className="card-play"><Glyph name="play" /></span></button><IconButton icon="more" label={`${station.title} options`} onClick={event => m.openMenu("station", event, station.id)} /></article>)}</Rail></Section>
    <Section title="Latest Episodes" onMore={() => m.go("schedule")}><Rail label="Latest episodes" className={styles.episodesRail}><div className="episode-grid">{episodes.map(([show,title,description],index) => <article className="episode" key={show}><button type="button" aria-label={`Listen to ${title}`} onClick={() => m.go("station:1")}><Art art={crop("4cb8f3aa",286+Math.floor(index/2)*378.5,392+(index%2)*115,92,92)} label={title!} /></button><div><small>{show}</small><button type="button" onClick={() => m.go("station:1")}>{title}</button><p>{description}</p></div><IconButton icon="more" label={`Options for ${title}`} onClick={event => m.openMenu("station", event, "station-1")} /></article>)}</div></Rail></Section>
    <Section title="Artists Take Over" onMore={() => m.go("category:Radio takeovers")}><Rail label="Artists take over" className="square-rail">{["Tyler White","Fancy Hagood","SIENNA SPIRO","The Cowgays","Shaboozey"].map((name,index) => <StationCard key={name} station={{id:`takeover-${index}`,title:name,destination:"station:1",art:crop("4cb8f3aa",286+index*227,675,208,208)}} />)}</Rail></Section>
