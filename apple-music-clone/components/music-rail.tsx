...
export function Rail({ children, className = "", label, initialIndex = 0, onPositionChange }: {
  ...
  return <div className={`rail-wrap ${className}`} data-rail={label}>
    ...
    <div className="rail-arrows">
      <IconButton icon={(className.includes("poster-rail") || className.includes("feature-rail") || className.includes("song-rail")) ? "rail-back" : "back"} label={`Previous ${label}`} aria-controls={id} disabled={!edges.previous} onClick={() => move(-1)} />
      <IconButton icon={(className.includes("poster-rail") || className.includes("feature-rail") || className.includes("song-rail")) ? "rail-next" : "chevron"} label={`Next ${label}`} aria-controls={id} disabled={!edges.next} onClick={() => move(1)} />
    </div>
  </div>;
}
