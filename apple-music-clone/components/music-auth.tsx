...
export function AuthenticationDialog() {
  ...
  const finish = () => {
    ...
    window.history.replaceState(window.history.state, "", "/?view=new");
...
export function CheckoutDialog() {
  ...
  const [checkoutStep, setCheckoutStep] = useState(m.scene.formStep ?? 0);
  const [error, setError] = useState("");
  const timer = useRef<ReturnType<typeof setTimeout> | null>(null);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const frame = requestAnimationFrame(() => {
      const host = scroll.current;
      const target = host?.querySelector<HTMLElement>("#checkout-confirm");
      if (host && target) host.scrollTop += target.getBoundingClientRect().top - host.getBoundingClientRect().top - 28;
    });
    return () => cancelAnimationFrame(frame);
  }, [done]);
  const updateCheckoutStep = () => {
    ...
    const relativeTop = (id: string) => {
      const target = host.querySelector<HTMLElement>(`#${id}`);
      return target ? target.getBoundingClientRect().top - hostTop : Number.POSITIVE_INFINITY;
    };
    setCheckoutStep(relativeTop("checkout-confirm") <= 160 ? 2 : relativeTop("checkout-billing") <= 160 ? 1 : 0);
  };
  const submit = (event: FormEvent) => {
    event.preventDefault(); setError("");
    ...
    setPending(true);
    timer.current = setTimeout(() => { setPending(false); setDone(true); setCheckoutStep(3); m.patch({ checkout: false, guest: true, namedProfile: false, formStep: 3 }); m.setLibrary(l => ({ ...l, cancelled: false })); }, 450);
  };
  return <Dialog title="Payment Method" onClose={() => m.patch({ overlay: null, checkout: false, guest: false })} className={`capture-checkout ${checkoutStep === 1 ? "checkout-billing-step" : ""} ${checkoutStep === 3 ? "checkout-confirmed-step" : ""}`}><span className="dialog-apple"><Glyph name="apple" size={28} /></span><form onSubmit={submit} className="checkout-scroll" ref={scroll} onScroll={updateCheckoutStep} autoComplete="off"><div className="checkout-column">
