...
function ExpandedLyricsButton({ label, pressed, onClick }: { label: string; pressed: boolean; onClick: () => void }) {
  return <button type="button" className="icon-button expanded-lyrics-toggle" aria-label={label} title={label} aria-pressed={pressed} onClick={onClick}>
    <svg className="expanded-lyrics-glyph" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
      <path d="M7.2 2.8h9.6a4.4 4.4 0 0 1 4.4 4.4v7.4a4.4 4.4 0 0 1-4.4 4.4H10l-4.8 3.3.8-3.8a4.4 4.4 0 0 1-3.2-4.2V7.2a4.4 4.4 0 0 1 4.4-4.4Z" />
      <path fill="currentColor" stroke="none" d="M7.4 8.1h3.7v3.4c0 2-.9 3.4-2.9 4.2l-.8-1.2c1-.5 1.6-1.2 1.7-2H7.4V8.1Zm5.9 0H17v3.4c0 2-.9 3.4-2.9 4.2l-.8-1.2c1-.5 1.6-1.2 1.7-2h-1.7V8.1Z" />
    </svg>
  </button>;
}
...
export function Player() {
  ...
  const legacyDiscovery = m.scene.page === "new" && m.scene.catalog !== "queue" && Boolean(
    m.scene.catalog === "legacy" || m.scene.hero === "superbloom" || m.scene.source?.startsWith("e027fe6d"),
  );
  // Panels and non-legacy navigation editing own inline materials. Legacy
  // discovery keeps its catalog-scoped glass while the editor is open.
  const editingPlayerStyle = m.scene.panel
    ...
    : m.scene.editingNav && !legacyDiscovery
      ? { background: "rgb(249 249 251 / 50%)", backdropFilter: "blur(22px) saturate(1)", WebkitBackdropFilter: "blur(22px) saturate(1)" }
      : undefined;
