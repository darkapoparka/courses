...
export function LibraryView() {
  ...
  return <div className={`library-page faithful-library library-${page}`}>
    <div className="library-topbar" data-visible={showTitle || undefined}>
      ...
      {page !== "library" && page !== "playlists" && !empty && <button type="button" className="library-sort" aria-label={`Sort ${title}`} aria-haspopup="menu" onClick={event => m.openMenu("sort", event)}>{page === "made-for-you" ? "Playlist Type" : <Glyph name="sort" size={16} />}<Glyph name="down" size={12} /></button>}
    </div>
    {empty ? <EmptyState icon={page === "library" ? "apple-music" : "made-for-you-empty"} title={page === "library" ? "Add music to your library" : undefined} description={page === "library" ? "Browse millions of songs and collect your favourite here." : "Personal mixes that you add will appear here."} action={page === "library" ? "Browse Apple Music" : undefined} onAction={() => m.go("new")} /> : page === "songs" ? <MusicTrackTable tracks={tracks} compact /> : page === "artists" ? <div className="artist-library-split">
    ...
    <Footer compact={empty || page !== "made-for-you" && page !== "playlists"} />
  </div>;
}
