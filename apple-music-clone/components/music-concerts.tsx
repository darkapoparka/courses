...
function ConcertLocationGlyph({ size = 16, suggestion = false, selected = false }: { size?: number; suggestion?: boolean; selected?: boolean }) {
  return suggestion
    ? <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.8} strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M12 11v10"/><path d="M12 11a5 5 0 1 0 0-10 5 5 0 0 0 0 10Z" fill="currentColor"/><circle cx="12" cy="6" r="1.55" fill="white" stroke="none"/></svg>
    : <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor" stroke="none" aria-hidden="true"><path d="M12 22s7-6.6 7-12a7 7 0 1 0-14 0c0 5.4 7 12 7 12Z"/><circle cx="12" cy="10" r="2.4" fill={selected ? "var(--accent)" : "white"}/></svg>;
}
