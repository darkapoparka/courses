...
export function SettingsView() {
  ...
  return <AccountFrame>
    ...
    <section className="account-section account-access" id="account-access"><h2>{zh ? "账户访问" : "Account Access"}</h2><div>
      <h3>{zh ? "网页版 Music、播客和 TV" : "Music, Podcasts and TV on the Web"}</h3><p>{zh ? "你在网页浏览器中登录 Apple Music 后，也会自动登录 Apple TV 和 Apple 播客。这样你就可以在网页上播放电影、电视节目和播客。你随时可以在所有浏览器中退出登录所有服务。" : "When you sign in to Apple Music on a web browser, you are automatically signed in to Apple TV and Apple Podcasts. This allows you to play movies, TV shows and podcasts on the web. You can sign out of all services and all browsers at any time."}</p><button type="button" className="text-accent" onClick={() => { m.patch({ guest: true, namedProfile: false }); m.notify("Signed out of this preview. No external browser sessions were changed."); }}>{zh ? "退出所有浏览器" : "Sign Out of All Browsers"}</button>
