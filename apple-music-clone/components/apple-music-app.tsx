...
function MusicShell() {
  ...
  // The native cancellation surface is where the saved session finishes
  // resolving the Apple ID full name and its personal-playlist navigation.
  // This is visible page/session chrome; the library records themselves stay intact.
  const resolvedCancellationChrome = m.scene.page === "subscription" && m.scene.profileName === "Alex Smith";
  const showPlaylists = !singlePlaylistAccountChrome && !localizedSearchChrome && !hideEmptyLibrarySearchPlaylists && (replayChrome || fixturePlaylists || resolvedCancellationChrome || m.scene.radioEdition === "hits" || suggestLessMenuPlaylists || m.library.playlists.length > initialPlaylistCount);
  ...
  return <div className={`music-app ${m.scene.guest ? "guest-session" : "member-session"} ${m.scene.panel ? "with-player-panel" : ""} ${m.scene.checkout ? "checkout-stage" : ""}`} lang={zh ? "zh-Hans" : "en-SG"} data-scene={m.scene.page} data-replay-chrome={replayChrome || undefined} data-source={m.scene.source} data-flow={m.scene.flow} data-form-step={m.scene.formStep ?? 0}>
