...
function MusicShell() {
  ...
  const navigation = (page: string, label: string, icon: GlyphName) => <button type="button" key={page} className="sidebar-row" aria-current={activePage === page ? "page" : undefined} onClick={() => go(page)}><SidebarGlyph name={icon} size={17} /><span>{label}</span></button>;
