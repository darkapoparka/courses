...
function MusicShell() {
  ...
  return <div className={`music-app ${m.scene.guest ? "guest-session" : "member-session"} ${m.scene.panel ? "with-player-panel" : ""} ${m.scene.checkout ? "checkout-stage" : ""}`} data-scene={m.scene.page} data-source={m.scene.source}>
    ...
    {m.scene.guest && !m.scene.checkout && !m.scene.expanded && !m.scene.video && <div className="trial-banner"><div><strong>Get over 100 million songs free for 1 month.</strong><small>Plus your entire music library on all your devices. 1 month free, then $10.98/month.</small><span className="sr-only">Local reference preview. No payment or subscription will be created.</span></div><button type="button" onClick={() => m.patch({ overlay: "signin", formStep: 0 })}>Try It Free</button></div>}
