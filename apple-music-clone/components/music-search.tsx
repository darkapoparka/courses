...
export function SearchView() {
  ...
  useEffect(() => {
    ...
    const updateBottom = () => {
      ...
      setAtBottom(maximum > 1 && main.scrollTop >= maximum - 1);
    };
    const frame = requestAnimationFrame(updateBottom);
    const observer = new ResizeObserver(updateBottom);
    observer.observe(main);
    main.addEventListener("scroll", updateBottom, { passive: true });
    return () => { cancelAnimationFrame(frame); observer.disconnect(); main.removeEventListener("scroll", updateBottom); };
  }, []);
  ...
  const topResults: { title: string; subtitle: string; art: Artwork; round?: boolean; explicit?: boolean; action: () => void }[] = [
    ...
    { title: albumTitle, subtitle: "Album · Olivia Rodrigo", art: albumArt, explicit: true, action: () => m.go("album") },
