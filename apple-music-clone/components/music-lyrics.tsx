...
export function Lyrics({ panel = false }: { panel?: boolean }) {
  ...
  useLayoutEffect(() => {
    ...
    const target = panel ? (m.scene.catalog === "legacy" ? 191 : 188) : (active === 0 ? 348 : 326) * Math.min(1, element.clientHeight / 903);
    element.scrollTop += selected.getBoundingClientRect().top - element.getBoundingClientRect().top - target;
  }, [active, panel, supported, m.scene.catalog]);
  ...
  return <div ref={viewport} className={`lyric-viewport ${panel ? "panel-lyric-viewport" : ""}`} tabIndex={0} aria-label="Reference lyrics">
    <ol>{capturedLyrics.map((line, index) => {
      ...
      return <li key={`${index}-${line}`} data-distance={Math.min(3, distance)} style={{ "--lyric-distance": distance } as CSSProperties} data-before={index < active || undefined} data-after={index > active || undefined} aria-current={index === active ? "true" : undefined}>
